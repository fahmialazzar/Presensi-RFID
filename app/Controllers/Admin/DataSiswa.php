...
class DataSiswa extends BaseController
{
   ...
   protected $siswaValidationRules = [
      ...
      'tgl_lahir' => [         // Tambahkan validasi tanggal lahir
      ...
      'unique_code' => [
          'rules' => 'required|exact_length[10]|is_unique[tb_siswa.unique_code]',
          'errors' => [
   ...
   public function saveSiswa()
{
    ...
    $result = $this->siswaModel->createSiswa(
        ...
        tglLahir: $this->request->getVar('tgl_lahir'),    // Tambahkan tanggal lahir
...
public function updateSiswa()
{
    ...
    // Update unique_code validation rule if changed
    if ($siswaLama['unique_code'] != $this->request->getVar('unique_code')) {
        $this->siswaValidationRules['unique_code']['rules'] = 'required|exact_length[10]|is_unique[tb_siswa.unique_code]';
    } else {
        // If not changed, don't check uniqueness
        $this->siswaValidationRules['unique_code']['rules'] = 'required|exact_length[10]';
    }
    ...
    try {
        ...
        if (!empty($siswa)) {
            ...
            $data = [
                'result' => 0,
                'index' => $index,
                'message' => 'Data siswa tidak dapat diimpor'
            ];
            echo json_encode($data);
        }
    } catch (\Exception $e) {
