...
class QRGenerator extends BaseController
{
   ...
   public function generateQrSiswa()
   {
      ...
      $this->generate(
         unique_code: $this->request->getVar('nis'),
   ...
   public function generateQrGuru()
   {
      ...
      $this->generate(
         unique_code: $this->request->getVar('nuptk'),
         nama: $this->request->getVar('nama'),
         nomor: $this->request->getVar('nomor')
      );
   ...
   public function downloadAllQrSiswa()
   {
      try {
         ...
         if ($idKelas = $this->request->getVar('id_kelas')) {
            ...
            $dataSiswa = $siswaModel->getSiswaByKelas($idKelas);
         } else {
            // Ambil semua siswa
            $dataSiswa = $siswaModel->getAllSiswaWithKelas();
         }

         // Cek apakah ada data siswa
         if (empty($dataSiswa)) {
            session()->setFlashdata([
               'msg' => 'Tidak ada data siswa' . ($kelas ? " di kelas $kelas" : ''),
               'error' => true
            ]);
            return redirect()->back();
         }

         // BERSIHKAN FOLDER LAMA SEBELUM GENERATE
         $pathToBeCleaned = $this->qrCodeFilePath . "qr-siswa/" . ($kelas ? "{$kelas}/" : '');
         if (file_exists($pathToBeCleaned)) {
            $this->deleteDirectory($pathToBeCleaned);
         }
         
         // Buat folder baru
         if (!file_exists($pathToBeCleaned)) {
            mkdir($pathToBeCleaned, 0755, true);
         }

         // Generate QR code untuk setiap siswa
         foreach ($dataSiswa as $siswa) {
            $siswaKelas = $this->getKelasJurusanSlug($siswa['id_kelas']) ?? 'unknown';
            $siswaPath = $this->qrCodeFilePath . "qr-siswa/{$siswaKelas}/";
            
            if (!file_exists($siswaPath)) {
               mkdir($siswaPath, 0755, true);
            }

            // Simpan path lama
            $oldPath = $this->qrCodeFilePath;
            
            // Set path baru untuk siswa ini
            $this->qrCodeFilePath = $siswaPath;
   ...
   public function downloadAllQrGuru()
   {
      try {
         ...
         $dataGuru = $guruModel->getAllGuru();

         // Cek apakah ada data guru
         if (empty($dataGuru)) {
            session()->setFlashdata([
               'msg' => 'Tidak ada data guru',
               'error' => true
            ]);
            return redirect()->back();
         }

         // Set warna untuk guru
         $this->qrCode->setForegroundColor($this->foregroundColor2);
         $this->label->setTextColor($this->foregroundColor2);
         ...
         // BERSIHKAN FOLDER LAMA SEBELUM GENERATE
         if (file_exists($this->qrCodeFilePath)) {
            $this->deleteDirectory($this->qrCodeFilePath);
         }

         // Buat folder baru
         if (!file_exists($this->qrCodeFilePath)) {
            mkdir($this->qrCodeFilePath, 0755, true);
         }

         // Generate QR code untuk setiap guru
         ...
         // Buat file zip
         $output = self::UPLOADS_PATH . 'qrcode-guru.zip';
   ...
   /**
    * Hapus directory beserta isinya secara rekursif
    */
