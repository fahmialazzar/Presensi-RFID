...
class SiswaModel extends Model
{
   ...
   public function cekSiswaByNIS(string $nis)
{
    $this->join(
        'tb_kelas',
        'tb_kelas.id_kelas = tb_siswa.id_kelas',
    ...
    return $this->where(['tb_siswa.nis' => $nis])->first();
}
   ...
   public function createSiswa($nis, $nama, $idKelas, $jenisKelamin, $tglLahir,$alamat, $noHp, $uniqueCode)
   {
       return $this->save([
           'nis' => $nis,
           'nama_siswa' => $nama,
...
public function updateSiswa($id, $nis, $nama, $idKelas, $jenisKelamin,$tglLahir, $alamat, $noHp, $uniqueCode)
{
    return $this->save([
        $this->primaryKey => $id,
        'nis' => $nis,
    ...
    if (!empty($array)) {
        ...
        foreach ($array as $item) {
            if ($i == $index) {
                ...
                $jenisKelamin = getCSVInputValue($item, 'jenis_kelamin');
                if (strtolower($jenisKelamin) == 'laki-laki' || strtolower($jenisKelamin) == 'l') {
                    $data['jenis_kelamin'] = 'Laki-laki';
                } else if (strtolower($jenisKelamin) == 'perempuan' || strtolower($jenisKelamin) == 'p') {
                    $data['jenis_kelamin'] = 'Perempuan';
                } else {
                    $data['jenis_kelamin'] = $jenisKelamin; // Gunakan nilai asli jika tidak sesuai format
                }
                
                $data['tgl_lahir'] = getCSVInputValue($item, 'tgl_lahir');  // Tambahkan field ini
                $data['alamat'] = getCSVInputValue($item, 'alamat');
                $data['no_hp'] = getCSVInputValue($item, 'no_hp');
                $data['unique_code'] = getCSVInputValue($item, 'unique_code');
